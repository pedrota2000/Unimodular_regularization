...
class CustomBundleSolver1D(BundleSolver1D):
    def __init__(self, sofT_list, phim_list, all_nets, V_net, g1, TL=False, *args, **kwargs):
    ...
    def additional_loss(self,r,f,x, head):
        ...
        if self.TL == False and False:
            k = 2.5
            thres = 400000
            add_coef = (1/2) + (1/2)*np.tanh(k*(self.global_epoch - 800000)/thres)
     ...
     #       DDV = diff(diff(self.V_net[head](p), p), p)
      ...
      #      add_DV = ((DV - DV_th)**2).mean()
       #     add_DDV = ((DDV - DDV_th)**2).mean()
        ...
        g_det = 0
        flat_metric = 0
        if head == self.n_heads-1 and self.global_epoch%100 == 0 and False:
...
class NNholo_multihead_heist():
    ...
    def plot_separate_losses(self, xlabel= 'epochs', ylabel = r'$\log_{10}\mathcal{L}$', \
              figsize=(10,4), thick=0.8, left_x_lim=0, save_fig = False):
        
        trained_epochs = len(self.solver.metrics_history['train_loss'])
        
        loss_add = self.solver.metrics_history['add_loss']
        loss_train = self.solver.metrics_history['train_loss']
        loss_DE = np.array(loss_train)-np.array(loss_add)
        #print(loss_train)
        #print(loss_add)
    ...
    def MH_transfer(self, pretrained_path, sofT_path, lr=5e-5):
        ...
        solver_arch = master_dict['solver_arch']
        solver_head_arch = master_dict['solver_head_arch']
        V_arch = master_dict['V_arch']
        V_head_arch = master_dict['V_head_arch']
        
        V_body_NEW  = CustomNN(n_input_units = 1, hidden_units = V_arch[:-2] ,actv = nn.SiLU, n_output_units = V_arch[-1])
        V_body_NEW.load_state_dict(master_dict['state_V_body'])
        H_NEW = [FCNN(n_input_units=3, hidden_units=solver_arch[:-2],n_output_units = solver_arch[-1]) for _ in range(6)]
        for i in range(len(H_NEW)):
            H_NEW[i].load_state_dict(master_dict['body_dict_solver'][i])
        head_NEW = [FCNN(n_input_units=solver_arch[-1], hidden_units=solver_head_arch, n_output_units = 1)for _ in range(len(H_NEW))]
        self.nets_NEW = np.ones([len(H_NEW),1],dtype=nn.Module) # i -> equation, j -> head #
        V_head_NEW = FCNN(n_input_units=V_arch[-1], hidden_units=V_head_arch, n_output_units = 1)
        for i in range(len(H_NEW)):
        ...
        add_loss = master_dict['add_loss']
        prev_epoch = master_dict['epoch']
    #    prev_optim_state = torch.load(path, map_location=torch.device('cpu'))['optimizer']
     #   prev_optim = 
