...
class CustomBundleSolver1D(BundleSolver1D):
    def __init__(self, sofT_list, phim_list, all_nets, V_net, g1, metric_flat_coef=100, TL=False, *args, **kwargs):
        ...
        self.V_0_flag = int(input('Force V(0), dV(0),ddV(0) ? No[0], Yes[1]'))
...
class NNholo_multihead_heist():
    ...
    def __init__(self, saving_path, sofT_path_list=None, phim_list=None, delta = 0.0, curriculum = 1.0, u_pts = 48, solver_nets=[32,32,32],V_arch = [16,16,16,16], head_type = 'FCNN', solver_head_arch = [32,16],V_head_arch = [16,8], actv_V_head = nn.SiLU, u_sampling = 'chebyshev2-noisy', n_batches_train=1,load_path = None, metric_flat_coef = 100, transfer_sofT_path = None, transfer_lr=1e-3):
        ...
        if transfer_sofT_path != None:
            ...
            self.n_heads = 1
            self.MH_transfer(pretrained_path = load_path, sofT_path = transfer_sofT_path, lr=transfer_lr)

            self.nets=self.nets_NEW
            self.V_net = self.V_NEW
            self.sofT_list = [transfer_sofT_path]
    #        self.solver_nets = self.nets
    ...
    def plot_metric(self, vmax=1000, logscale=True, save_fig = True):
        ...
        for f in range(len(func)):
            ...
            for k in range(len(heads)):
               # fig1,ax = plt.subplots(1,2,figsize=(10,5))
                ...
                cbar.set_label('$\sqrt{|g|}$', fontsize=12)
#                fig1.suptitle('Function ' + str(func[f]) + ' for $\phi_M = $' + str(phim_param[k]))
                plt.title('Function ' + str(func[f]) + ' for $\phi_M = $' + str(phim_param[k]))
                plt.hlines(point,0,1,linewidth = 2.5, color = 'black',linestyle = 'dashed')
                plt.xlabel('u')
                plt.ylabel('index')
                # plt.plot(self.solver.Va_uh_list[k].cpu().detach().numpy(),self.solver.Sigma_uh_list[k].cpu().detach().numpy(),'.k')
                # plt.plot(self.solver.Va_uh_list[k][point].cpu().detach().numpy(),self.solver.Sigma_uh_list[k][point].cpu().detach().numpy(),'or')
                # plt.xlabel('$V_{a,h}$')
                # plt.ylabel('$\Sigma_{h}$')
                plt.show()
                ...
                fig3 = plt.figure()
                col = [f'C{i}' for i in range(heads)]
                for k in range(len(heads)):
                    plt.plot(self.solver.Va_uh_list[k].cpu().detach().numpy(),self.solver.Sigma_uh_list[k].cpu().detach().numpy(),'.', color=col[k], label = rf'$\phi_M={phim_param[k]}$')
                    plt.plot(self.solver.Va_uh_list[k][point].cpu().detach().numpy(),self.solver.Sigma_uh_list[k][point].cpu().detach().numpy(),'or')
                plt.xlabel('$V_{a,h}$')
                plt.ylabel('$\Sigma_{h}$')
    ...
    def plot_residuals_in_u(self, max_bound = 0.05, print_overbound = False, save_fig=True):
       ### RESIDUAL PLOTS
        ...
        for head in range(self.n_heads):
            ...
            if save_fig==True:
                fig.savefig(f'{self.path}/DE_res_1-3_head_{head}_epoch_{self.solver.global_epoch}.pdf')
        ...
        for head in range(self.n_heads):
            ...
            if save_fig==True:
                fig2.savefig(f'{self.path}/DE_res_4-7_head_{head}_epoch_{self.solver.global_epoch}.pdf')
    ...
    def MH_save_results(self,path):
        body_state = []
        head_state = np.ones([len(self.nets[:,0]),int(len(self.sofT_list))],dtype = object)
    ...
    def MH_transfer(self, pretrained_path, sofT_path, lr=5e-5):
        ...
        print( self.solver_nets, self.solver_head_arch)
        ...
        load_head = int(input('Load pretrained head(s)? No[0] ;  Yes[1]'))
        if load_head == 1:
            #print('len V_head_state', len(master_dict['V_head_state']))
            V_head_NEW.load_state_dict(master_dict['V_head_state'][0])
            for i in range(len(H_NEW)):
                #print('len H_head_state', len(master_dict['head_state']),len(master_dict['head_state'][i,:]))
                head_NEW[i].load_state_dict(master_dict['head_state'][i,0])
                
        self.nets_NEW = np.ones([len(H_NEW),1],dtype=nn.Module) # i -> equation, j -> head #


        unf = int(input('Unfreeze bodies? No[0] ;  Yes[1]'))
        if unf == 0:  #frozen bodies
            for i in range(len(H_NEW)):
                self.nets_NEW[i] = NET_FREEZE(H_NEW[i],head_NEW[i])
            self.V_NEW = [NET_FREEZE(V_body_NEW,V_head_NEW)]
            
        elif unf == 1:  #unfreeze the bodies
            for i in range(len(H_NEW)):
        ...
        b=int(input('Load prev optim? No[0] ;  Yes[1]'))
        if b==1:
